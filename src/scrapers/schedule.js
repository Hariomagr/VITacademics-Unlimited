  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      const schedule = table
        .slice(1) // table.slice(2, table.length - 11)
        .filter(
          row => Object.keys(row).length == 15 || Object.keys(row).length == 10
        )
        .map(row => {
